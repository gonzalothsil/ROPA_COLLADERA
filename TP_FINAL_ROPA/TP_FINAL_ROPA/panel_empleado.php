...
if (isset($_POST['agregar_producto'])) {
  ...
  //  $imagen = mysqli_real_escape_string($conn, trim($_POST['imagen'] ?? ''));
    ...
    if (isset($_FILES['imagen']) && $_FILES['imagen']['error'] === UPLOAD_ERR_OK) {
        ...
        $nuevo_nombre_archivo = uniqid('prod_', true) . '.' . $file_extension;
        $destino = $directorio_subidas . $nuevo_nombre_archivo;

        // Mover el archivo subido de la ubicación temporal a la permanente
        if (move_uploaded_file($file_tmp, $destino)) {
            $imagen_ruta = mysqli_real_escape_string($conn, $destino); // Guardar la ruta relativa
        } else {
            $mensaje = "❌ Error al mover el archivo subido.";
        }
    } else if (isset($_FILES['imagen']) && $_FILES['imagen']['error'] !== UPLOAD_ERR_NO_FILE && $_FILES['imagen']['error'] !== UPLOAD_ERR_OK) {
        // Manejo de otros errores de subida
        $mensaje = "❌ Error de subida de archivo (código: " . $_FILES['imagen']['error'] . ").";
    }

    if ($nombre !== '' && $precio > 0) {
        $sql = "INSERT INTO productos (nombre, precio, stock, imagen, color, talle)
                VALUES ('$nombre', $precio, $stock, '$imagen_ruta', '$color', '$talle')";
        if ($conn->query($sql)) {
    ...
    <div class="card-body">
      <form method="POST" enctype="multipart/form-data" class="row g-3">
        ...
        <div class="col-md-6 mt-2">
          <input type="file" name="imagen" class="form-control" accept="image/*" placeholder="URL imagen (opcional)">
        </div>
